...
const groupEntriesByRenderTime = (entry: PerformanceEventTiming) => {
    ...
    let group: pendingEntriesGroup | undefined
    ...
    for (let i = pendingEntriesGroups.length - 1; i >= 0; i--) {
        ...
        if (Math.abs(renderTime - (potentialGroup?.renderTime || 0)) <= 8) {
...
const cleanupEntries = () => {
    ...
    for (let i = 0; i < pendingEntriesGroups.length; i++) {
        ...
        if (group?.startTime !== undefined && group?.processingEnd !== undefined) {
...
const attributeINP = (metric: INPMetric): INPMetricWithAttribution => {
    ...
    if (!firstEntry) {
        throw new Error('No entries found in INP metric')
    }

    const group = entryToEntriesGroupMap.get(firstEntry)

    const processingStart = firstEntry.processingStart
    const processingEnd = group?.processingEnd ?? firstEntry.processingEnd

    // Sort the entries in processing time order.
    const processedEventEntries =
    ...
    const attribution: INPAttribution = {
        ...
        loadState: getLoadState(firstEntry.startTime || 0),
    }
