...
const attributeFID = (metric: FIDMetric): FIDMetricWithAttribution => {
    ...
    const attribution: FIDAttribution = {
        eventTarget: getSelector(fidEntry?.target || null),
        eventType: fidEntry?.name || '',
        eventTime: fidEntry?.startTime || 0,
        eventEntry: fidEntry as unknown as PerformanceEventTiming,
        loadState: getLoadState(fidEntry?.startTime || 0),
    }
